 ...
 * do projétil são feitos na mão (mais simples que estender Projectile e ainda
 * compatível com o GeckoLib). As animações ({@code idle} / {@code throw}) ficam no
...
public class BoneArmorSansEntity extends Entity implements GeoEntity {
    ...
    private void onHit(ServerLevel server, @Nullable LivingEntity victim, Vec3 where) {
        ...
        server.sendParticles(net.minecraft.core.particles.ParticleTypes.POOF,
                where.x, where.y, where.z, 8, 0.2, 0.2, 0.2, 0.02);
        discard();
    }
    ...
    protected void defineSynchedData() {
        this.entityData.define(DATA_MODE, Mode.THROWN.ordinal());
    }
