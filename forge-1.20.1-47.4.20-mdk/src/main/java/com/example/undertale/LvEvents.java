...
public class LvEvents {
     ...
     *
     * O EXP por kill segue a tabela do Undertale (ver {@code lv.txt}):
     * animais passivos rendem pouco (1–5), monstros comuns médio (10–20),
     * monstros perigosos alto (50–100) e chefes sobem 3–4 níveis de uma vez.
     */
    @SubscribeEvent
    ...
    /** EXP concedido por matar um mob comum, conforme a categoria (ver {@code lv.txt}). */
     ...
     * jogador que ataca, mais dano ele causa. Assim a mesma faca que farma LV acompanha
     * o crescimento até o LV máximo.
     *
     * Aplicado no {@link LivingHurtEvent} (servidor) para ler o LV real do NBT e para o
     * bônus ainda passar pela redução de armadura do alvo, como o dano normal.
     * O {@code the_real_knife} não é afetado — ele já dá dano infinito.
     */
    @SubscribeEvent
    public static void onRealKnifeHit(LivingHurtEvent event) {
        if (event.getEntity().level().isClientSide()) return;
     ...
     * então a grade ainda está cheia: lemos a receita, devolvemos exatamente 1 set de
     * ingredientes e zeramos o resultado. Resultado prático: o item "monta" mas é
     * revertido na hora, sem o jogador perder material.
     */
    @SubscribeEvent
    public static void onGatedCraft(PlayerEvent.ItemCraftedEvent event) {
        ...
        if (grid instanceof CraftingContainer craftGrid) {
            ...
            level.getRecipeManager()
                    ...
                    .ifPresent(recipe -> {
                        for (Ingredient ing : recipe.getIngredients()) {
                            ...
                            ItemStack[] opcoes = ing.getItems();
                            if (opcoes.length == 0) continue;
                            ItemStack devolver = opcoes[0].copy();
                            devolver.setCount(1);
